...
namespace MatchThree 
{
    namespace Model
    {
        Block::Block(): m_id(-1),
                        ...
                        m_screen_current_position(vector2D(-1)),
                        m_screen_target_position(vector2D(-1))
        {}
        ...
        bool Block::isPossibleTarget(int target_height) const
        {
            ...
            */
            ...
            return m_screen_current_position.r <= new_possible_position;
        }
        ...
        void Block::setBlockFree()
        {
            ...
            m_free_fall = false;
            m_state = Controller::BlockState::DEAD;
            m_screen_current_position = m_screen_target_position = vector2D(-1);
        }
        ...
        {
            m_screen_current_position.c = Controller::BOARD_POS_C + column * Controller::BLOCK_SIZE_C;
            m_screen_current_position.r += (row_direction * Controller::BLOCK_SIZE_R);
        }

        void Block::setScreenCurrentPosition(const vector2D position, Index2D direction)
        {
            m_screen_current_position.c = position.c + direction.c*Controller::BLOCK_SIZE_C;
            m_screen_current_position.r = position.r + direction.r * Controller::BLOCK_SIZE_R;
        }

        void Block::setScreenTargetPosition(const Index2D& target_index)
        {
            m_screen_target_position.c = Controller::BOARD_POS_C + target_index.c * Controller::BLOCK_SIZE_C;
            m_screen_target_position.r = Controller::BOARD_POS_R + target_index.r * Controller::BLOCK_SIZE_R;
        }
        ...
        {
            m_screen_current_position.c += (Controller::BLOCK_SIZE_C * direction);

            m_screen_target_position.c = BOARD_POS_C + target_Index.c * BLOCK_SIZE_C;
            m_screen_target_position.r = BOARD_POS_R + target_Index.r * BLOCK_SIZE_R;
        }
        ...
        void Block::spwan(const Index2D& start_index, const Index2D& target_index, unsigned int current_time)
        {
            ...
            m_screen_current_position.c = Controller::BOARD_POS_C + start_index.c * Controller::BLOCK_SIZE_C;
            m_screen_current_position.r = Controller::BOARD_POS_R + start_index.r * Controller::BLOCK_SIZE_R;
        ...
        void Block::simulate(Index2D target_index, unsigned int current_time)
        {
            if (isFalling())
            {
                ...
                m_screen_current_position.r += (0.5f * Controller::GRAVITY * delta_t * delta_t * booster);
                m_current_falling_time = current_time;

                if (m_screen_current_position.r >= m_screen_target_position.r)
                {
                    m_screen_current_position.r = m_screen_target_position.r;
        ...
        {
            return  { static_cast<int>((position.c - Controller::BOARD_POS_C) / static_cast<float>(Controller::BLOCK_SIZE_C)),
                      static_cast<int>((position.r - Controller::BOARD_POS_R) / static_cast<float>(Controller::BLOCK_SIZE_R)) };
        }

        vector2D indexToPosition(Index2D index)
        {
            return { Controller::BOARD_POS_C + index.c * Controller::BLOCK_SIZE_C,
                     Controller::BOARD_POS_R + index.r * Controller::BLOCK_SIZE_R };
        }
    } 
}