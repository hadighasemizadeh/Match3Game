...
namespace MatchThree {
    namespace Model
    {
        Board::Board():   m_start_Point(vector2D(-1)),
                          m_board_state(BoardState::SPWANNING),
                          m_generated_index{Index2D(-1,-1),Index2D(-1,-1)}
        {                                 
                        ...
                        const int block_id)
        {
            ...
            processed[index.c][index.r] = true;
            out.push_back(index);
            ...
            while (!q.empty())
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    if (isSafe(grid, c + col[k], r + row[k], processed, block_id))
                    {
                        ...
                        out.push_back(Index2D(c + col[k], r + row[k] ));
                        q.push(Index2D(c + col[k], r + row[k]));
                    }
                }
            }
        }
        ...
        void Board::findMatches(const grid& grid, Index2D start_index, std::vector<Index2D>& out)
        {
            ...
            if (start_index == Index2D(-1))
                return;

            const int block_id = grid[start_index.c][start_index.r].getBlockID();
        ...
        int Board::removeMatches() 
        {
            ...
            for (auto& index : m_generated_index)
            {
                ...
                if (dead_list.size() >= MINIMUM_NUMBER_OF_MATCHES)
                {
                    ...
                    for (auto matchIndex : dead_list)
                    {
                        ...
                        m_grid[matchIndex.c][matchIndex.r].setBlockFree();
                    }
            ...
            if (total_matches == 0 && black_holes.size()>0)
            {
                ...
                {
                    if (m_grid[itr->c][itr->r].getBlockID() == -1 || *itr == Index2D(-1))
                    {
        ...
        void Board::applyGravity()
        {
            ...
            {
                int height = 1;
                for (int r = hole.r - 1; r >= 0; r--)
                {
                    if (m_grid[hole.c][r].getBlockID() == -1)
                    {
                        height++;
                    }
                    else
                    {
                        Index2D new_target_index(hole.c, r + height);

                        m_grid[hole.c][r].fallInto(new_target_index);

                        //std::cout << Index2D(hole.c,r) << " Fall Into target: " << new_target_index;
                        //std::cout << "ScreenTargetPosition: " << m_grids[hole.c][r].getScreenTargetPosition();

                        swap(Index2D(hole.c, r), new_target_index);
        ...
        {
            int diff_x = abs(index1.c - index2.c);
            int diff_y = abs(index1.r - index2.r);
            return ((0 == diff_x && 1 == diff_y) || (1 == diff_x && 0 == diff_y));
        }
        
        bool Board::isWall(Index2D index) const
        {
            return  index.c < 0 || index.c >= m_grid.size() ||
                    index.r < 0 || index.r >= m_grid[0].size();
        }
        ...
        void Board::freeFall()
        {
            ...
            if (m_generated_index[0] == Index2D(-1) ||
                m_generated_index[1] == Index2D(-1))
            {
                return;
            }

            m_grid[m_generated_index[0].c][m_generated_index[0].r].freeFall();

            if (m_generated_index[1] != Index2D(-1))
                m_grid[m_generated_index[1].c][m_generated_index[1].r].freeFall();
        }

        void Board::resetInPair()
        {
            m_generated_index[0] = m_generated_index[1] = vector2D(-1);
        }
        ...
        {
            const auto tmp_block1 = m_grid[v1.c][v1.r];
            const auto tmp_block2 = m_grid[v2.c][v2.r];
            m_grid[v1.c][v1.r] = tmp_block2;
            m_grid[v2.c][v2.r] = tmp_block1;
        }
        ...
        {
            if (tail == Index2D(-1) || head == Index2D(-1))
            {
            ...
            pair_idx[0].c += direction;
            pair_idx[1].c += direction;

            if (pair_idx[0].c < 0 || pair_idx[0].c >= m_grid.size() || 
                pair_idx[1].c < 0 || pair_idx[1].c >= m_grid.size())
            {
            ...
            if (pair_idx[0].r == pair_idx[1].r)
            {
                if (direction > 0 && pair_idx[0].c < pair_idx[1].c ||
                    direction < 0 && pair_idx[0].c > pair_idx[1].c)
                {
                    int new_head_row = firstAvialableRowInColumn(pair_idx[1].c);
                    if (pair_idx[1].r > new_head_row)
                    {
                        std::cout << "Cannot jump!\n";
                        return { { -1,-1 }, { -1,-1 } };
                    }

                    pair_idx[1].r = new_head_row;
                    pair_idx[0].r = head.r;
                } 
                else
                {
                    int tail_new_row = firstAvialableRowInColumn(pair_idx[0].c);
                    if (pair_idx[0].r > tail_new_row)
                    {
                        std::cout << "Cannot jump!\n";
                        return { { -1,-1 }, { -1,-1 } };
                    }

                    pair_idx[0].r = tail_new_row;
                    pair_idx[1].r = tail.r;
                }               
            }
            else // They were in the same column
            {
                ...
                {
                    int new_head_row = firstAvialableRowInColumn(pair_idx[1].c);
                    if (pair_idx[0].r > head.r || pair_idx[1].r > new_head_row)
                    {
                        std::cout << "Cannot jump!\n";
                        return { { -1,-1 }, { -1,-1 } };
                    }
                    ...
                    pair_idx[1].r = new_head_row;
                    pair_idx[0].r = new_head_row - 1;
                }
                else // Tail is in front
                {
                    int tail_new_row = firstAvialableRowInColumn(pair_idx[0].c);
                    if (pair_idx[1].r > tail.r || pair_idx[0].r > tail_new_row)
                    {
                        std::cout << "Cannot jump!\n";
                        return { { -1,-1 }, { -1,-1 } };
                    }
                    pair_idx[0].r = tail_new_row;
                    pair_idx[1].r = tail_new_row - 1;
                }
            }
        ...
        void Board::spawnRandomNewBlock(unsigned int current_time)
        {
            ...
            if (m_generated_index[0] == Index2D(-1) || m_generated_index[1] == Index2D(-1))
            {
            ...
            int head_row = firstAvialableRowInColumn(m_generated_index[1].c);
            m_grid[m_generated_index[1].c][head_row].spwan( m_generated_index[1],
                                                            Index2D(m_generated_index[1].c, head_row),
                                                            current_time);
            m_generated_index[1].r = head_row;

            // find avialable target row for tail and spwan it
            int tail_row = firstAvialableRowInColumn(m_generated_index[0].c);
            m_grid[m_generated_index[0].c][tail_row].spwan( m_generated_index[0],
                                                            Index2D(m_generated_index[0].c, tail_row),
                                                            current_time);
            m_generated_index[0].r = tail_row;
        ...
        void Board::filpFallingBlocks()
        {
            ...
            Index2D current_state = pair_idx[0] - pair_idx[1];
            ...
            Index2D next_index{ pair_idx[1] + neighbors[next_state_idx] };
            ...
            // By default
            int tail_row = m_generated_index[0].r;
            int head_row = m_generated_index[1].r;
            ...
            if (next_state_idx == 2)
            {
                tail_row = head_row;
                head_row--;
                swap(m_generated_index[1], Index2D(m_generated_index[1].c, head_row));
            }
            else
            {
                if (next_state_idx == 3)
                ...
                tail_row = firstAvialableRowInColumn(next_index.c);
            }

            // Validate head's and tail's new row
            if (tail_row < pair_idx[0].r || tail_row < 0 ||
                head_row < pair_idx[1].r || head_row < 0)
            {
            ...
            // Update Tail
            Index2D target_index_tail{ next_index.c, tail_row };
            ...
            m_grid[m_generated_index[0].c][m_generated_index[0].r].setScreenCurrentPosition(target_pos, neighbors[next_state_idx]);
            m_grid[m_generated_index[0].c][m_generated_index[0].r].setScreenTargetPosition(target_index_tail);

            // Update Head | Head column never changes
            if (next_state_idx == 3)
                swap(Index2D(m_generated_index[1].c, head_row), m_generated_index[1]);

            m_generated_index[1].r = head_row;
            // Update head's target screen positions
            m_grid[m_generated_index[1].c][m_generated_index[1].r].setScreenTargetPosition(m_generated_index[1]);
        }
        ...
        void Board::slide(int direction)
        {
            ...
            if (new_target_indeces[0] == Index2D(-1) || new_target_indeces[1] == Index2D(-1))
            {
            ...
            int order = 1;
            if (new_target_indeces[0].c != new_target_indeces[1].c)
            {
                if (direction > 0)
                {
                    order = (new_target_indeces[0].c > new_target_indeces[1].c ? 0 : 1);
                }
                else
                {
                    order = (new_target_indeces[0].c > new_target_indeces[1].c ? 1 : 0);
                }
            }
            ...
            m_grid[new_target_indeces[0].c][new_target_indeces[0].r] = tmp_tail;
            gridAt(m_generated_index[0]).setBlockFree();
            m_generated_index[0] = new_target_indeces[0];
            
            m_grid[new_target_indeces[1].c][new_target_indeces[1].r] = tmp_head;
        ...
        void Board::applyAnimation(unsigned int current_time)
        {
            ...
            for (int c = 0; c < m_grid.size(); c++)
            {
                ...
                for (int reversed_row = m_grid[0].size() - 1; reversed_row >= 0; reversed_row--)
                {
                    m_grid[c][reversed_row].simulate(Index2D(c, reversed_row), current_time);
