...
namespace MatchThree 
{
    namespace View
    {
        ...
        Renderer::Renderer(): m_window(nullptr), m_renderer(nullptr)
        {
            ...
            m_window = SDL_CreateWindow("Match three game",
        ...
        void Renderer::drawFilledRectangle(Model::vector2D pos, int w, int h)
        {
            SDL_Rect rect = { pos.c, pos.r, w, h};
            SDL_RenderFillRect(m_renderer, &rect);
        }
        
        void Renderer::drawText(const char* text, Model::vector2D pos, SDL_Color color)
        {
            ...
            SDL_Rect rect = { pos.c, pos.r };
            SDL_QueryTexture(texture, nullptr, nullptr, &rect.w, &rect.h);
            SDL_RenderCopy(m_renderer, texture, nullptr, &rect);
            SDL_DestroyTexture(texture);
        }

        void Renderer::drawTexture(unsigned int texture_id, Model::vector2D pos)
        {
            validateTexture(texture_id);
            SDL_Rect rect = { pos.c, pos.r};
        ...
        void Renderer::drawTextureCentered(unsigned int texture_id, Model::vector2D pos, int w, int h, double scale)
        {
            validateTexture(texture_id);
            ...
            SDL_Rect rect;
            rect.x = pos.c + static_cast<int>((w - texture_width)/2);
            rect.y = pos.r + static_cast<int>((h - texture_height)/2);
        ...
        void Renderer::drawButton(std::string btn_text, Model::vector2D pos, int w, int h)
        {
            setColor(255, 255, 255, 255);
            drawFilledRectangle(pos, w, h);
            drawText(btn_text.c_str(),
                     pos + Model::vector2D(static_cast<int>(w/6), static_cast<int>(h /6)),
                     {0, 0, 150, 255});
        }
        ...
        void Renderer::setClipRect(Model::vector2D pos, int w, int h)
        {
            SDL_Rect rect = { pos.c, pos.r, w, h};
